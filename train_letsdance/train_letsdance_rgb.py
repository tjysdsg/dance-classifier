...
from train_letsdance.common import CATEGORIES, CATEGORY_TO_ID
...
ORIGINAL_FPS = 30
n_frames = 16
data_dir = 'letsdance-preprocessed'
...
class _DataModule(TemporalDataModule):
    def __init__(self, data_dir: str, batch_size: int = 32, test_size=0.3, val_size=0.1, n_consecutive_frames=6,
                 fps=30, *args, **kwargs, ):
        self.vid2label = {}
        self.vid2fid = {}
        self.vids = []
    ...
    def read_data_dir(self):
        for cat_dir in os.scandir(self.data_dir):  # categories
            if cat_dir.is_dir():
                ...
                vids = []
                for f in os.scandir(cat_dir.path):  # video frames inside one category
                    filename: str = f.name
                    split = filename.replace('_rendered', '').split('_')
                    fid = int(os.path.splitext(split[-1])[0])

                    # we can assume all vids are unique, but I don't want to take chances
                    vid = f'{category}_' + '_'.join(split[:-1])

                    if vid not in self.vid2fid:
                        self.vid2fid[vid] = []
...
def train():
    ...
    trainer = Trainer(
        gpus=1,
        max_epochs=200, callbacks=[checkpoint_callback, early_stop_callback],
